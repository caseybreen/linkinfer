test_that("weight_ipw logistic method works", {
  ...
  # Add linked indicator to population
  population$linked <- linked_flag
  result <- weight_ipw(
...
test_that("weight_ipw ml method works", {
  ...
  result <- weight_ipw(
    ...
    method = "ml",
    ranger_args = list(num.trees = 100)
  )
...
test_that("weight_ipw stabilized vs unstabilized", {
  ...
  population$linked <- linked_flag
  stab <- weight_ipw(population, linked_indicator = "linked", covariates = "x", stabilize = TRUE)
  unstab <- weight_ipw(population, linked_indicator = "linked", covariates = "x", stabilize = FALSE)

  # Stabilized weights should have mean closer to 1
  expect_true(abs(mean(stab$weights) - 1) < abs(mean(unstab$weights) - 1) + 1)
})

test_that("weight_diagnostics works for IPW", {
  set.seed(42)
  n <- 500
  ...
  population$linked <- linked_flag
  ipw_result <- weight_ipw(population, linked_indicator = "linked", covariates = "age")
  diag <- weight_diagnostics(ipw_result$weights, population, linked, covariates = "age")

  expect_s3_class(diag, "linkinfer_weight_diagnostics")
...
test_that("print methods work", {
  ...
  population$linked <- linked_flag
  result <- weight_ipw(population, linked_indicator = "linked", covariates = "x")
  out <- paste(capture.output(print(result), type = "message"), collapse = " ")
  expect_true(grepl("Inverse|Weight|Method", out))

  diag <- weight_diagnostics(result$weights, population, linked, covariates = "x")
